 ...
 *
 * Ported verbatim from the original implementation (legacy/js/rula-tables.js).
 * A parity test (tests/rula-tables-parity.spec.ts) loads the legacy file and
 * asserts these tables are byte-for-byte identical. Never edit these values
 * by hand — they come from McAtamney & Corlett (1993), "RULA: a survey method
 * for the investigation of work-related upper limb disorders",
 * Applied Ergonomics 24(2), 91-99.
 *
 * Table A: upper limb posture scores. Key digits: upper arm (incl.
 *          adjustments), lower arm, wrist, wrist twist.
 * Table B: neck, trunk and leg posture scores. Key digits: neck, trunk
 *          (each incl. adjustments), legs.
 * Table C: grand score from Score C (arm & wrist total) and Score D
 *          (neck, trunk & leg total), keyed "CxD".
 */

export const TABLE_A: Readonly<Record<string, number>> = {
...
export const TABLE_B: Readonly<Record<string, number>> = {
...
export const TABLE_C: Readonly<Record<string, number>> = {
